...
os.environ['CUDA_VISIBLE_DEVICES'] = "3" # decide which GPU
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser() # model_epoch_7.pt
    parser.add_argument('-img_dir', default="./road/",type=str, help='input dataset directory')
    # parser.add_argument('-model_path',default="./output/model_best.pt", type=str, help='trained model path')
    parser.add_argument('-model_path', default="./weights_res101/model_best.pt", type=str, help='trained model path') # model_epoch_20 best model_unet_vgg_16_best
    parser.add_argument('-model_type',default="resnet101", type=str, choices=['vgg16', 'resnet101', 'resnet34'])
    parser.add_argument('-out_viz_dir',default="./road_visual/", type=str, required=False, help='visualization output dir')
    parser.add_argument('-out_pred_dir',default="./road_result/", type=str, required=False,  help='prediction output dir')
    ...
    for path in tqdm(paths):
        ...
        mask_c = (prob_map_full * 255).astype(np.uint8)
        mask_c = cv.resize(mask_c, None, fx=0.2, fy=0.2, interpolation=cv.INTER_AREA)
        # mask_c = cv2.GaussianBlur(mask_c, (15, 15), 0)
        # mask_bgr = cv.applyColorMap(mask_c, cv.COLORMAP_JET)

        # cv.imshow('img0', mask_c)
        # cv.waitKey()
        # print("prob_map_full",prob_map_full)

        if args.out_pred_dir != '':
            cv.imwrite(filename=join(args.out_pred_dir, f'{path.stem+"_mask"}.jpg'), img=(prob_map_full * 255).astype(np.uint8))
        ...
        if args.out_viz_dir != '':
            ...
            ax.imshow(img_0)
            ax.imshow(prob_map_viz_full, alpha=0.2)
